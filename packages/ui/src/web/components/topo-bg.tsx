/**
 * Topographic contour background. Concentric bezier loops in `info`
 * blue at low alpha, with one highlighted loop in `accent.action`.
 * Lines draw in on mount via the `ds-draw-path` keyframe.
 *
 * Used as the editorial backdrop for the opener / hero sections.
 */
export function TopoBg() {
  return (
    <div
      aria-hidden
      className="ds-animate-breathe pointer-events-none absolute inset-0 overflow-hidden"
    >
      <svg
      ...
      >
        <defs>
          <radialGradient id="topo-vignette" cx="50%" cy="55%" r="62%">
        ...
        <g mask="url(#topo-mask)" fill="none" strokeWidth="1.2">
          <path
            ...
            className="ds-animate-draw-path"
          />
          <path
        ...
        <g transform="translate(600 470)">
          <circle r="3" fill="var(--color-accent-action)" />
          <circle r="10" stroke="var(--color-accent-action)" strokeOpacity="0.4" fill="none" />
          <circle r="20" stroke="var(--color-accent-action)" strokeOpacity="0.18" fill="none" />
        </g>
      </svg>
    </div>
  );
